...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        if(find(key) != end()){		//method for finding duplicates on geekforgeeks
        	cerr << "Warning: Attempt to insert duplicate key '" << key << "'." << endl;
        	return;
        }
        //interpreted from psuedo-code on canvas
        while(x != nullptr){
        	y = x;
        	if(key < x->key()){
        		x = x->left;
        ...
        z->right = nullptr;
        z->color = RED;
        insert_fixup(z);	//call fixup to make sure rbt properties hold
        size_++;		//increment size of tree after inserting
    }
...
private:
    ...
    void delete_tree(Node<K, V> *n) {
    	if (n != nullptr) {		//delete node if not null
    		delete_tree(n->left);
    		delete_tree(n->right);
    		delete n;
    	}
    	size_--;	//decrement size after removing node
    }
    ...
    int height(Node<K, V> *node) const {
    	if(node == nullptr){	//subtract 1 if node is null
    		return -1;
    	}
    ...
    size_t leaf_count(Node<K, V> *node) const {
    	if(node == nullptr){	//don't add anything if node is null
        	return 0;
        }
        if(node->left==nullptr && node->right==nullptr){	//add 1 if node has no children
        	return 1;
        }
    ...
    size_t internal_node_count(Node<K, V> *node) const {
    	if(node==nullptr){		//don't add anything if node is null
    		return 0;
    	}else if(node->left==nullptr && node->right==nullptr){	//don't add anything if node has no children
    		return 0;
    	}
    	return 1 + internal_node_count(node->left) + internal_node_count(node->right);		//only add 1 if node has a child
    }
    ...
    int diameter(Node<K, V> *node) const {
    	if(node == nullptr){	//don't add anything if null is node
    		return 0;
    	}
    ...
    size_t width(Node<K, V> *node, size_t level) const {
    	if(node==nullptr){		//add 0 if node is null
    		return 0;
    	}else if(level==0){		//if level is 0, width is 1
    		return 1;
    	}
    ...
    size_t null_count(Node<K, V> *node) const {
    	if(node == nullptr){	//add 1 if node is null
    		return 1;
    	}
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
    	if(node == nullptr){	//add 0 if node is null, add level otherwise
    		return 0;
    	}
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
    	if(node == nullptr){	//add level is node is null, add 0 otherwise
    		return level;
    	}
