...
size_t lomuto_partition(int array[], size_t left, size_t right) {
    // performs lomuto partition
	int p = array[left];		//pivot
	size_t s = left;
...
int quick_select(int array[], size_t left, size_t right, size_t k) {
    // performs quickselect, utilizing Lomuto Partitioning
	size_t s = lomuto_partition(array, left, right);
	if(s == k-1){		//if s is the kth smallest element, return that value
		return array[s];
	}else if(left - right == 0){	//if left and right are equal, then return value at index
		return array[right];
	}else if(s > k -1){			//if s is greater than kth smallest, only keep left side of array
		return quick_select(array, left, s-1, k);
	}else{				//if s is smaller than kth smallest, only keep right side of array
...
int main(int argc, char *argv[]) {
    ...
    //convert values to appropriate data type for quick_select call
    int *values_arr = new int[num_values];
    values_arr = values.data();
    size_t length = num_values;
    size_t k_val = k;

    // call the quick_select function and display the result
    int qs = quick_select(values_arr, length, k_val);
    cout << "Smallest element " << k << ": " << qs << endl;
